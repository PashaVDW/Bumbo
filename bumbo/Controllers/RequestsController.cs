        ...
        public async Task<IActionResult> Index(RequestsViewModel oldModel, string searchTerm, int page = 1)
        {
            ...
            var requests = new List<Request>()
            {
                new Request()
                {
                    ...
                    Message = "Ik wil vakantie, want ik vind mijn collega 's vervelend"
                }
            };
            ...
            var htmlTableTwo = tableBuilderTwo.GenerateTable("Uitgaande Aanvragen", headersTwo, requests, "", item =>
            {
                var emp = _branchesRepository.GetEmployeeById(item.EmployeeId);
                return $@"
                 <td class='py-2 px-4'>{emp.FirstName} {emp.MiddleName} {emp.LastName}</td>
                 ...
                 <td class='py-2 px-4 text-right'>
                 <button onclick=""window.location.href='../Requests/Read?requestId={item.RequestToBranchId}'"">✏️</button> 
                 </td>";
